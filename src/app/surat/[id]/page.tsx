...
export default function apa({ params }: { params: { id: number } }) {
  const [coba, setCoba] = useState("");

  const { data, isLoading } = getAyat(params.id);
  useEffect(() => {
    if (!isLoading) {
  ...
  const hasil = () => {
    ...
    const home = (
      <>
        <h1 className=" text-3xl md:text-6xl text-center pt-6 px-6  text-[#094067]">
